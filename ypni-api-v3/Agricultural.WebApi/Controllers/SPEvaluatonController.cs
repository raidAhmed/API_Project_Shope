...
namespace Agricultural.WebApi.Controllers
{
    ...
    [ApiController]
    public class SPEvaluatonController : BaseApiController
    {
        [HttpGet]
        ...
        [HttpPost("Post")]
        public async Task<IActionResult> Post([FromBody] ProviderEvaluationDto entity)
        {
            ...
            try
            {
                var ressu = await ServiceManager.ProviderEvaluationService.Find(x => x.UserId == entity.UserId && x.ServiceProviderId == entity.ServiceProviderId);

                if (ressu.Data != null)
                {
                    entity.Id = ressu.Data.FirstOrDefault().Id;
                    var result = await ServiceManager.ProviderEvaluationService.Update(entity);
                ...
                {
                    var result = await ServiceManager.ProviderEvaluationService.Add(entity);
