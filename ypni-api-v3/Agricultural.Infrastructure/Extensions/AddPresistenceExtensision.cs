    ...
    public static class AddPresistenceExtensision
    {
        public static IServiceCollection AddPresistence(this IServiceCollection services, IConfiguration configuration)
        {
          ...
          options =>
          {
              ...
              options.Password.RequireDigit = false;// identityOptionsConfig.RequiredDigit;
              options.Password.RequireLowercase = false;// identityOptionsConfig.RequireLowercase;
              options.Password.RequiredUniqueChars = 0;// identityOptionsConfig.RequiredUniqueChars;
              options.Password.RequireUppercase = false;// identityOptionsConfig.RequireUppercase;
            ...
            services.ConfigureApplicationCookie(options =>
            {
                ...
                options.AccessDeniedPath = "/UserAccess/AccessDenied";
            ...
            services.AddTransient(provider =>
                  new Lazy<IDirectorateRepository>(provider.GetService<IDirectorateRepository>)); 
            services.AddTransient(provider =>
                  new Lazy<IMarketsRepository>(provider.GetService<IMarketsRepository>)); 
            services.AddTransient(provider =>
                  new Lazy<INewsRepository>(provider.GetService<INewsRepository>));
            services.AddTransient(provider =>
         ...
         new Lazy<ISP_MainClassificationRepository>(provider.GetService<ISP_MainClassificationRepository>));
           ...
           new Lazy<ISpecialSectionsRepository>(provider.GetService<ISpecialSectionsRepository>));
            ...
            services.AddTransient(provider => new Lazy<IProduct_Variant_AttributeRepository>(provider.GetService<IProduct_Variant_AttributeRepository>));
